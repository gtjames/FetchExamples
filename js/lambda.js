	...
	document.getElementById("search").addEventListener("click", () => searchQuote());

	let row = 0;

	function getQuoteList() {
		//fetch API for all records
		fetch(apiURL, {} )
	...
	function searchQuote() {
		//fetch API to search for a speaker
		fetch(apiURL+`/search?speaker=${speaker.value}`, { })
			.then(response => response.json())
			.then(quotes => { showQuotes(quotes)})
			.catch(err => console.log('Fetch Error :', err) );
	}
	...
	function createQuote() {
		...
		fetch(apiURL, {
			method: 'PUT',				//	id is generated on the server side
