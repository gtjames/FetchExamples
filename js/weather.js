...
function getWeather() {
    ...
    fetch(url)
        ...
        .then(weather => {                  //  with the resulting JSON data do something
            sunset.innerText  = `Sunset: ${niceTime(weather.city.sunset, 0)}`;
            sunrise.innerText = `Sunrise: ${niceTime(weather.city.sunrise, 0)}`;
// 435-896-4814    Keldon
// 417-887-4990    American National
            ...
            for (let day of weather.list) {
                ...
                <div class="grid-item w3-theme-${(color%2)>0 ? 'l2':'d2'}">
                    ...
                    <p>Forecast: ${day.weather[0].description} <img src='http://openweathermap.org/img/wn/${day.weather[0].icon}@2x.png' alt="" height="20%"></p>
                </div>`;
            }
