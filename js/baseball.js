...
function showStats(teamStats) {
    ...
    html = `<tr><th>${teamStats.parameters.season} Season</th><th>Games Played</th><th>Wins</th><th>Percent Wins</th><th>Loses</th><th>Percent Loses</th>`;
    html += `
		<tr><td>Home:</td><td>${games.played.home}</td><td>${games.wins.home.total}</td><td>${(games.wins.home.percentage*100).toFixed(0)}%</td><td>${games.loses.home.total}</td><td>${(games.loses.home.percentage*100).toFixed(0)}%</td><td>The ${teamStats.response.team.name}                 <img src=${teamStats.response.team.logo}    width=100px height=70px></td></tr>
		<tr><td>Away:</td><td>${games.played.away}</td><td>${games.wins.away.total}</td><td>${(games.wins.away.percentage*100).toFixed(0)}%</td><td>${games.loses.away.total}</td><td>${(games.loses.away.percentage*100).toFixed(0)}%</td><td>play in the ${teamStats.response.league.name}       <img src=${teamStats.response.league.logo}  width=100px height=70px></td></tr>
		<tr><td>All: </td><td>${games.played.all} </td><td>${games.wins.all.total} </td><td>${(games.wins.all.percentage *100).toFixed(0)}%</td><td>${games.loses.all.total} </td><td>${(games.loses.all.percentage *100).toFixed(0)}%</td><td>which is in the ${teamStats.response.country.name}  <img src=${teamStats.response.country.flag} width=100px height=70px></td></tr>`;
